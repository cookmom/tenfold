...
export function construct4Fold(R) {
  ...
  steps.push({
    ...
    elements: [
      ...
      ...pts8.map(p => ({ type: 'line', x1: 0, y1: 0, x2: p.x, y2: p.y, stroke: C(0.08), sw: 0.4 })),
      ...pts8.map(p => ({ type: 'circle', cx: p.x, cy: p.y, r: 2.5, stroke: 'none', fill: C(0.3) })),
  ...
  steps.push({
    ...
    elements: [
      ...
      { type: 'circle', cx: firstInter.x, cy: firstInter.y, r: 2.5, stroke: 'none', fill: C(0.4) }
    ]
  })
  ...
  const cornerArcs = outerCorners.map((c, i) => {
    ...
    return { type: 'circle', cx: c.x, cy: c.y, r: rSmall * 2, stroke: C(0.08), fill: 'none', sw: 0.4 }
  })
  steps.push({
